...
func NewFTPStorage(u *url.URL) (StorageBackend, error) {
	...
	client, err := goftp.DialConfig(config, host+":"+port)
	if err != nil {
...
func (f *ftpStorage) Upload(ctx context.Context, path string, data io.Reader) error {
	// Create full remote path
	remotePath := filepath.Join(f.basePath, path)

	// Ensure remote directory exists
	if err := f.ensureRemoteDir(filepath.Dir(remotePath)); err != nil {
		return fmt.Errorf("failed to ensure remote directory: %w", err)
	}

	// Upload file
	err := f.client.Store(remotePath, data)
	if err != nil {
...
func (f *ftpStorage) DeleteOldBackups(ctx context.Context, maxCount int) error {
	...
	for _, file := range toDelete {
		...
		remotePath := filepath.Join(f.basePath, file)
